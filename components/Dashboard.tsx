  ...
  return (
    <div className="p-4 md:p-8 bg-background min-h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                ...
                {todaysAppointments.length === 0 && (
                  <tr>
                    <td colSpan={3} className="p-4 text-center text-text-secondary">{t('dashboard.noAppointments')}</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">{t('dashboard.recentPatients')}</h2>
          <ul>
            {patients.slice(0, 5).map(patient => (
              <li key={patient.id} className="flex items-center justify-between p-2 rounded-md hover:bg-gray-50 cursor-pointer" onClick={() => onSelectPatient(patient.id)}>
                ...
                <button className="text-brand-primary hover:text-brand-dark">{t('dashboard.view')}</button>
              </li>
            ))}
          </ul>
        </div>
